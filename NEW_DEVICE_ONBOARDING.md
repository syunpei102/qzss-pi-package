...
```
QZSS_SERIAL_PORT=/dev/ttyUSB0        # 手順2で確認したポート
QZSS_CLOUD_URL=https://eq.shum10.com/ingest
QZSS_INGEST_TOKEN=<Cloud Run側のINGEST_TOKENと同じ値>
QZSS_DEVICE_ID=pi-shibuya-01          # 手順1で決めた拠点ID
DISCORD_WEBHOOK_URL=<温度異常等の通知用。DISCORD_SETUP.md参照>
```

`QZSS_INGEST_TOKEN` はデプロイ担当者に確認する(公開リポジトリのため
このファイルに実際の値を書かないこと)。

## 4. サービス化する(OTA更新・状態報告・自動復旧をまとめて有効化)
...
## 5. 拠点がCloud Run側に認識されるのを待つ(最大1時間+α)

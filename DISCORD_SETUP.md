...
の両方を行う。前者はWebhook(ボット不要)、後者はDiscord Bot(コマンドを
受け取れるアプリ)が必要になる — 別々の仕組みなので両方設定する。
...
`/reboot`・`/update_check`・`/set_region` を実行できるようにする。
...
設定後、Discordのチャンネルで `/reboot`・`/update_check`・`/set_region`
と入力すると、`device`(拠点ID)・`prefecture`(都道府県名、`set_region`
のみ)を入力補完付きで選べる。実行結果は自分にだけ見えるメッセージで
返る。
